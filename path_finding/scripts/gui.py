...
class Application:
  ...
  def run(self):
    ...
    # self.obs_surf.mask = pg.mask.from_surface(self.image)
  ...
  def load_obstacles(self):
    try:
      self.obs_surf.image = pg.image.load(IMAGE_NAME).convert()
    except:
      print("the file was not found in the same directory.")
      return
    else:
